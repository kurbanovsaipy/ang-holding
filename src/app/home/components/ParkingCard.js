...
export default function ParkingCard({el}) {
    return (
        <div className="card">
            ...
            />
            <a href={`/#/catalog/house/${el?.id}/smallGrid?filter=property.status:AVAILABLE`} className='cover_link'></a>
            <div className="project_name">{el?.projectName}</div>
            <a href={`/#/catalog/house/${el?.id}/smallGrid?filter=property.status:AVAILABLE`} className='title'>{el?.title || 'Название'}</a>
